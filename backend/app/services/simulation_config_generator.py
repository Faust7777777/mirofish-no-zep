...
class SimulationConfigGenerator:
    ...
    def _summarize_entities(self, entities: List[EntityNode]) -> str:
        ...
        # 按类型分组（兼容 EntityNode：实际类型存放在 labels 列表中）
        by_type: Dict[str, List[EntityNode]] = {}
        for e in entities:
            t = (getattr(e, 'entity_type', None)
                 or (e.labels[0] if getattr(e, 'labels', None) else None)
                 or 'Unknown')
            if t not in by_type:
                by_type[t] = []
            by_type[t].append(e)
...
- reasoning (string): 简要说明为什么这样配置"""
    ...
    def _generate_event_config(self, context, simulation_requirement, entities):
        logger.info("开始生成事件配置和热点话题...")

        def _entity_type_of(e):
            return (getattr(e, 'entity_type', None)
                    or (e.labels[0] if getattr(e, 'labels', None) else None)
                    or 'Person')

        entity_types_available = list({_entity_type_of(e) for e in entities})
        type_examples = {}
        for e in entities:
            etype = _entity_type_of(e)
        ...
        prompt = (
            ...
            f"## 上下文\n{context_truncated}\n\n"
            f"## 要求\n"
            f"- initial_posts 必须包含1-3个触发事件的初始帖子\n"
            f"- 第一个帖子应该是Outsider类型，内容是悄悄传递禁诗或提及禁诗存在\n"
            f"- 帖子内容要口语化、自然，不要像官方通知\n"
            f"- hot_topics 列出3-5个会因此引发讨论的话题关键词\n"
        )
        system_prompt = (
            f"你是社交媒体模拟专家。根据模拟需求生成事件配置，返回纯JSON格式，不要加markdown代码块。\n\n"
            f"返回JSON格式如下：\n"
            f"{{\"initial_posts\": [{{\"content\": \"帖子内容\", \"poster_type\": \"Outsider\", \"platform\": \"reddit\"}}], "
            f"\"hot_topics\": [\"话题1\"], \"narrative_direction\": \"舆论引导方向\", \"reasoning\": \"配置说明\"}}\n\n"
            f"重要：initial_posts里必须包含触发事件的核心帖子，内容要符合模拟需求的具体场景。\n\n"
        ...
        try:
            ...
            return {"hot_topics": [], "narrative_direction": "", "initial_posts": [], "reasoning": "默认"}
    ...
    def _assign_initial_post_agents(
        ...
        type_aliases = {
            "outsider": ["outsider", "mediaoutlet", "person"],
    ...
    def _generate_agent_configs_batch(
        ...
        for i, entity in enumerate(entities):
            ...
            config = AgentActivityConfig(
                agent_id=agent_id,
                entity_uuid=getattr(entity, 'uuid', str(agent_id)),
                entity_name=entity.name,
                entity_type=getattr(entity, 'entity_type', None) or (entity.labels[0] if entity.labels else 'Unknown'),
                activity_level=rule_config.get('activity_level', 0.5),
                posts_per_hour=rule_config.get('posts_per_hour', 1.0),
                comments_per_hour=rule_config.get('comments_per_hour', 2.0),
                active_hours=rule_config.get('active_hours', list(range(8, 23))),
                response_delay_min=rule_config.get('response_delay_min', 5),
                response_delay_max=rule_config.get('response_delay_max', 60),
                sentiment_bias=rule_config.get('sentiment_bias', 0.0),
                stance=rule_config.get('stance', 'neutral'),
                influence_weight=rule_config.get('influence_weight', 1.0)
            )
    ...
    def _generate_agent_config_by_rule(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        # EntityNode 的真实类型存放在 labels[0]，回退到 'Unknown'
        entity_type = (
