...
class ZepToolsService:
    ...
    def __init__(self, api_key: Optional[str] = None, llm_client: Optional[LLMClient] = None):
        ...
        self.use_local_graph = not Config.USE_ZEP
    ...
    def _should_use_local_graph(self, graph_id: str) -> bool:
        return self.use_local_graph or graph_id.startswith("local_")

    def _get_local_graph_data(self, graph_id: str) -> Dict[str, Any]:
        return GraphBuilderService.get_local_graph_data(graph_id)

    @staticmethod
    def _get_profile_name(profile: Dict[str, Any], fallback: str = "Agent") -> str:
        return (
    ...
    def _extract_search_terms(query: str) -> List[str]:
        ...
        stop_terms = {
            "什么", "怎么", "如何", "为何", "为什么", "哪些", "哪个", "是否", "对于",
            "关于", "有关", "相关", "情况", "影响", "变化", "态度", "看法", "观点",
            "反应", "问题", "事件", "过程", "原因", "结果", "进行", "一个", "一些",
            "the", "and", "for", "with", "that", "this",
        }
        split_chars = "的了和与及对在将把被从向于就而并且或等所"
        terms: List[str] = []
        seen = set()

        def add_term(term: str) -> None:
            candidate = term.strip().lower()
            if len(candidate) < 2 or candidate in stop_terms or candidate in seen:
                return
            seen.add(candidate)
        ...
        for token in re.findall(r'[a-z0-9_-]{2,}|[\u4e00-\u9fff]{2,}', normalized):
            ...
            if re.fullmatch(r'[\u4e00-\u9fff]{2,}', token):
                parts = [part for part in re.split(f"[{split_chars}]+", token) if len(part) >= 2]
                for part in parts:
                    add_term(part)
                max_ngram = 3 if len(token) > 6 else 2
                for size in range(2, max_ngram + 1):
                    for idx in range(0, len(token) - size + 1):
                        add_term(token[idx:idx + size])

        return terms[:48]

    @classmethod
    def _score_text_match(cls, text: str, query: str, search_terms: List[str]) -> int:
        if not text:
            return 0
        ...
        for term in search_terms:
            if term in text_lower:
                ...
                score += min(12 + len(term) * 3, 36)

        if len(matched_terms) >= 2:
            score += len(matched_terms) * 8

        query_chars = {ch for ch in query_lower if '\u4e00' <= ch <= '\u9fff'}
        if query_chars:
            text_chars = {ch for ch in text_lower if '\u4e00' <= ch <= '\u9fff'}
    ...
    @staticmethod
    def _extract_key_quotes(response_text: str) -> List[str]:
        ...
        paired += re.findall(r'\u300c([^\u300c\u300d]{15,100})\u300d', clean_text)
        return [q for q in paired if not re.match(r'^[，,；;：:、]', q)][:5]

    @staticmethod
    def _normalize_history_response(response: Any) -> str:
    ...
    def _load_simulation_config(self, simulation_id: str) -> Dict[str, Any]:
    ...
    def _collect_local_interview_facts(
        self,
        graph_id: str,
        agent_name: str,
        ...
        queries = [
            f"{agent_name} {interview_requirement}".strip(),
            interview_requirement,
            simulation_requirement[:120].strip(),
            f"{agent_name} {simulation_requirement[:80]}".strip(),
        ]
        facts: List[str] = []
        seen = set()
    ...
    def _generate_local_interview_response(
        self,
        ...
        agent_name = self._get_profile_name(profile)
        history_lines = []
        for item in history_items[:4]:
            response = self._normalize_history_response(item.get("response"))
            history_lines.append(
                f"- [{item.get('platform', 'unknown')}/{item.get('timestamp', '')}] "
                f"问：{item.get('prompt', '')[:80]} 答：{response[:220]}"
            )

        facts_text = "\n".join(f"- {fact[:240]}" for fact in local_facts[:8]) or "- 暂无直接命中的图谱事实"
        history_text = "\n".join(history_lines) or "- 暂无历史采访记录"
        questions_text = "\n".join(f"{idx + 1}. {question}" for idx, question in enumerate(interview_questions))

        system_prompt = """你要扮演社会模拟中的一个角色，基于给定的人设、背景事实和历史发言生成离线采访回答。

要求：
1. 必须严格站在该角色的第一人称立场回答，保持人设一致
2. 只能输出自然语言回答，不要输出 JSON、工具调用或 Markdown 标题
3. 按“问题1：”“问题2：”逐条回答，每题 2-4 句
4. 优先引用提供的图谱事实和历史发言；资料不足时要明确说明“基于我目前所知”
5. 不要编造具体统计数字、外部来源或未提供的情节"""
...
"""
        ...
        try:
            ...
            logger.warning(f"离线采访生成失败，使用模板降级: {e}")
            fallback_basis = local_facts[0] if local_facts else (profile.get("bio") or profile.get("persona") or "我所处的环境对这件事非常敏感。")
            answers = []
            for idx, question in enumerate(interview_questions, 1):
                answers.append(
                    f"问题{idx}：从我的立场看，{fallback_basis[:120]}。"
                    f"这也决定了我会如何理解“{question}”。"
                    f"基于我目前所知，我更关注这件事对我所在群体和城邦秩序造成的影响。"
                )
            return "\n\n".join(answers)

    def _run_local_interviews(
        self,
        simulation_id: str,
        ...
        sim_config = self._load_simulation_config(simulation_id)
        graph_id = sim_config.get("graph_id", "")
        interviews: List[AgentInterview] = []

        for index, profile in zip(selected_indices, selected_agents):
            agent_name = self._get_profile_name(profile, fallback=f"Agent_{index}")
            agent_role = profile.get("profession", "未知")
            agent_bio = profile.get("bio", "")
            history_items = SimulationRunner.get_interview_history(
                simulation_id=simulation_id,
                agent_id=index,
                limit=6
            )
            local_facts = self._collect_local_interview_facts(
                graph_id=graph_id,
                agent_name=agent_name,
                interview_requirement=interview_requirement,
                simulation_requirement=simulation_requirement or sim_config.get("simulation_requirement", ""),
                limit=8
            )
            response_text = self._generate_local_interview_response(
                profile=profile,
    ...
    def search_graph(
        ...
        if self._should_use_local_graph(graph_id):
        ...
        try:
            if scope in ["edges", "both"]:
                ...
                for edge in all_edges:
                    score = (
                        self._score_text_match(edge.fact, query, search_terms)
                        + self._score_text_match(edge.name, query, search_terms)
                    )
                    if score > 0:
            ...
            if scope in ["nodes", "both"]:
                ...
                for node in all_nodes:
                    score = (
                        self._score_text_match(node.name, query, search_terms)
                        + self._score_text_match(node.summary, query, search_terms)
                    )
                    if score > 0:
            ...
            if not facts and scope == "edges":
                fallback_nodes = self._local_search(graph_id, query, limit=limit, scope="nodes")
                facts.extend(fallback_nodes.facts[:limit])
                nodes_result.extend(fallback_nodes.nodes[:limit])
    ...
    def get_node_detail(self, node_uuid: str) -> Optional[NodeInfo]:
        ...
        if self.use_local_graph:
            for graph_path in os.listdir(GraphBuilderService.LOCAL_GRAPH_DIR) if os.path.isdir(GraphBuilderService.LOCAL_GRAPH_DIR) else []:
                graph_id = os.path.splitext(graph_path)[0]
...
返回JSON格式的子问题列表。"""
    ...
    def panorama_search(
        ...
        def relevance_score(fact: str) -> int:
            return self._score_text_match(fact, query, search_terms)
    ...
    def quick_search(
        ...
        search_scope = "both" if self._should_use_local_graph(graph_id) else "edges"
    ...
    def interview_agents(
        ...
        # Step 4: 本地模式下直接走离线采访，不依赖运行中的模拟环境
        if not Config.USE_ZEP:
            result.interviews = self._run_local_interviews(
                simulation_id=simulation_id,
                selected_agents=selected_agents,
        ...
        try:
            ...
            for i, agent_idx in enumerate(selected_indices):
                ...
                agent_name = self._get_profile_name(agent, fallback=f"Agent_{agent_idx}")
                ...
                interview = AgentInterview(
                    ...
                    response=response_text,
                    key_quotes=self._extract_key_quotes(f"{twitter_response} {reddit_response}")
                )
    ...
    def _select_agents_for_interview(
        ...
        for i, profile in enumerate(profiles):
            summary = {
                "index": i,
                "name": self._get_profile_name(profile, fallback=f"Agent_{i}"),
