...
def prepare_simulation():
    ...
    # 既然上面已经获取过 data 和 simulation_id 了，下面就直接用
    try:
        ...
        if not state:
            logger.warning(f"准备模拟时找不到模拟状态: simulation_id={simulation_id}")
            return jsonify({
                "success": False,
                "error": t('api.simulationNotFound', id=simulation_id)
            }), 404
            
        # ... 接下来的代码保持原样 (force_regenerate 等) ...
        # 检查是否强制重新生成
        ...
        # 获取模拟需求；使用了热配置场景时允许为空（场景描述会作为兜底）
        simulation_requirement = project.simulation_requirement or ""
        scene_config_name_from_data = data.get('scene_config_name')
        if not simulation_requirement and not scene_config_name_from_data:
            return jsonify({
                "success": False,
        ...
        # 热配置：前端选中的场景名，可选；为空时使用默认《理想国》场景
        scene_config_name = data.get('scene_config_name')
...
@simulation_bp.route('/<simulation_id>/config/agent/<int:agent_id>/sentiment-bias', methods=['POST'])
def update_agent_sentiment_bias(simulation_id: str, agent_id: int):
    """更新单个 Agent 的情感倾向。"""
    try:
        data = request.get_json() or {}

        if "sentiment_bias" not in data:
            return jsonify({
                "success": False,
                "error": "缺少 sentiment_bias"
            }), 400

        updated = manager.update_agent_sentiment_bias(
            simulation_id=simulation_id,
            agent_id=agent_id,
            sentiment_bias=data.get("sentiment_bias")
        )
        ...
        logger.error(f"更新 Agent 情感倾向失败: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
