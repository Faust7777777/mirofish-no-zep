...
from recent_context import RecentContextInjector
...
async def run_twitter_simulation(
    ...
    # 注入"最近发言"上下文的辅助器（修复 Bug 2：角色之间无互动）
    twitter_context_injector = RecentContextInjector(db_path, platform="twitter")
    ...
    for round_num in range(total_rounds):
        ...
        # 把最近的 Twitter 发言注入活跃 Agent 的 system_message
        with twitter_context_injector.inject(agent for _, agent in active_agents):
...
async def run_reddit_simulation(
    ...
    # 注入"最近发言"上下文的辅助器（修复 Bug 2：角色之间无互动）
    reddit_context_injector = RecentContextInjector(db_path, platform="reddit")
    ...
    for round_num in range(total_rounds):
        ...
        actions = {agent: LLMAction() for _, agent in active_agents}
        # 把最近的 Reddit 发言注入活跃 Agent 的 system_message
        with reddit_context_injector.inject(agent for _, agent in active_agents):
            await result.env.step(actions)
