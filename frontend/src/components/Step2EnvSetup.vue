    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 0, 'completed': phase > 0 }">
        ...
        <div class="card-content">
          ...
          <div v-if="simulationId" class="info-card">
            ...
            <div class="info-row">
              <span class="info-label">场景配置</span>
              <span class="info-value">
                <span v-if="selectedSceneName" class="scene-tag">{{ selectedSceneName }}</span>
                <span v-else class="scene-tag default">默认《理想国》</span>
                <button class="scene-edit-btn" type="button" @click="openSceneConfig">
                  ⚙ 编辑/切换场景
                </button>
              </span>
            </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <div v-if="phase === 0" class="prepare-action-panel">
            <div class="prepare-action-copy">
              <span class="prepare-action-label">当前准备场景</span>
              <span class="prepare-action-value">{{ selectedSceneName || '默认《理想国》' }}</span>
              <p class="prepare-action-desc">
                先确认场景配置，再开始准备模拟。当前选择会直接用于这一次 Agent 人设和配置生成。
              </p>
            </div>
            <button
              class="action-btn primary prepare-start-btn"
              type="button"
              :disabled="!canStartPrepare"
              @click="startPrepareSimulation"
            >
              {{ isCheckingPreparedState || isStartingPrepare ? '准备中...' : '开始准备模拟' }}
            </button>
          </div>
                ...
                >
                  ...
                  <div class="agent-params">
                    ...
                    <div class="param-group">
                      ...
                      <div class="param-item sentiment-editor-item">
                        <span class="param-label">{{ $t('step2.sentimentBias') }}</span>
                        <div class="sentiment-editor">
                          <input
                            v-model.number="editableSentimentBiases[agent.agent_id]"
                            type="range"
                            min="-1"
                            max="1"
                            step="0.1"
                          ...
                          >
                          <div class="sentiment-editor-footer">
                            <span class="sentiment-scale">-1</span>
                            <span
                              class="param-value"
                              :class="getSentimentBiasClass(editableSentimentBiases[agent.agent_id])"
                            >
                              {{ formatSentimentBias(editableSentimentBiases[agent.agent_id]) }}
                            </span>
                            <span class="sentiment-scale">+1</span>
                          </div>
                          <button
                            class="mini-save-btn"
                            type="button"
                            :disabled="savingSentimentAgentIds[agent.agent_id] || !hasSentimentBiasChanged(agent)"
                            @click="saveAgentSentimentBiasValue(agent)"
                          >
                            {{ savingSentimentAgentIds[agent.agent_id] ? '保存中...' : '保存' }}
                          </button>
                        </div>
                      </div>
                      <div class="param-item">
...
import {
  ...
  updateAgentSentimentBias
} from '../api/simulation'
import SceneConfigPanel from './SceneConfigPanel.vue'
...
const editableSentimentBiases = ref({})
const savingSentimentAgentIds = ref({})
const selectedProfile = ref(null)
const showProfilesDetail = ref(true)
const isStartingPrepare = ref(false)
const isCheckingPreparedState = ref(false)
...
// 热配置：选中的场景名（为空时使用默认《理想国》场景）
const selectedSceneName = ref('')
const showSceneConfig = ref(false)
function openSceneConfig() {
  showSceneConfig.value = true
}
function onSceneApplied(sceneName) {
  selectedSceneName.value = sceneName
  addLog(`[场景] 已应用场景配置：${sceneName}（本次准备模拟将使用此场景）`)
}

const syncEditableSentimentBiases = (config) => {
  const nextValues = {}
  const agentConfigs = config?.agent_configs || []
  agentConfigs.forEach((agent) => {
    nextValues[agent.agent_id] = Number(agent.sentiment_bias ?? 0)
  })
  editableSentimentBiases.value = nextValues
}

watch(
  () => simulationConfig.value,
  (config) => {
    syncEditableSentimentBiases(config)
  }
)
...
const canStartPrepare = computed(() => {
  return Boolean(props.simulationId) && phase.value === 0 && !isStartingPrepare.value && !isCheckingPreparedState.value
})
...
const clampSentimentBias = (value) => {
...
const formatSentimentBias = (value) => {
  const normalized = clampSentimentBias(value)
  return `${normalized > 0 ? '+' : ''}${normalized.toFixed(1)}`
}

const getSentimentBiasClass = (value) => {
  const normalized = clampSentimentBias(value)
...
const hasSentimentBiasChanged = (agent) => {
  const draftValue = clampSentimentBias(editableSentimentBiases.value[agent.agent_id])
  const currentValue = clampSentimentBias(agent.sentiment_bias)
  return Math.abs(draftValue - currentValue) >= 0.001
}

const saveAgentSentimentBiasValue = async (agent) => {
  if (!props.simulationId) {
    addLog('[配置] 缺少 simulationId，无法保存情感倾向')
    return
  }

  const agentId = agent.agent_id
  const nextValue = clampSentimentBias(editableSentimentBiases.value[agentId])
  editableSentimentBiases.value[agentId] = nextValue
  savingSentimentAgentIds.value[agentId] = true

  try {
    const res = await updateAgentSentimentBias(props.simulationId, agentId, nextValue)
    ...
    if (res.data.config) {
      ...
      const targetAgent = simulationConfig.value?.agent_configs?.find(item => item.agent_id === agentId)
      if (targetAgent) {
        targetAgent.sentiment_bias = nextValue
      }
      editableSentimentBiases.value[agentId] = nextValue
    }

    addLog(`[配置] 已保存 ${agent.entity_name} 的情感倾向：${formatSentimentBias(nextValue)}`)
  } catch (err) {
    addLog(`[配置] 保存 ${agent.entity_name} 的情感倾向失败：${err.message}`)
  } finally {
    savingSentimentAgentIds.value[agentId] = false
  }
}
...
const startPrepareSimulation = async () => {
  ...
  try {
    ...
    const res = await prepareSimulation({
      ...
      // 热配置：用户在面板里选中的场景；为空时后端自动用默认《理想国》场景
      scene_config_name: selectedSceneName.value || undefined,
    })
...
const restorePreparedState = async () => {
  ...
  isCheckingPreparedState.value = true
  ...
  try {
    ...
    if (res.success && res.data && (res.data.already_prepared || res.data.status === 'ready')) {
      addLog(t('log.detectedExistingPrep'))
      await loadPreparedData()
    }
  } catch (err) {
    console.warn('检查已有准备状态失败:', err)
  } finally {
    isCheckingPreparedState.value = false
  }
}
